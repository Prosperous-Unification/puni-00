...
import type { RemoveRoleOutcome, RoleRefusal, RoleService } from '../service/role.service';
 ...
 */
const statusFor = (reason: RoleRefusal): number =>
 ...
 * is — a role belongs to one project and is addressed through it.
 *
 * Reading the roles stays on `GET /api/projects/:id`, which already answers
 * with them. A second list route would be a second read of one fact, and
 * clients would drift over which one is current.
 */
export function roleController(auth: AuthService, roles: RoleService) {
  return new Elysia({ prefix: '/api/projects' })
    .post(
      ...
      async ({ params, body, headers, set }) => {
        ...
        const outcome = await roles.add(params.id, user.id, body.name);
        if (!outcome.ok) {
          set.status = statusFor(outcome.reason);
          return { error: outcome.reason };
        }
        return { role: outcome.result };
      },
      { body: named },
    )
    .patch(
      ...
      async ({ params, body, headers, set }) => {
        ...
        const outcome = await roles.rename(params.id, params.roleId, user.id, body.name);
        if (!outcome.ok) {
          set.status = statusFor(outcome.reason);
          return { error: outcome.reason };
        }
    ...
    .delete('/:id/roles/:roleId', async ({ params, query, headers, set }) => {
      const user = await userFromHeaders(auth, headers);
      if (user === null) {
        set.status = 401;
        return { error: 'unauthenticated' };
      ...
      const outcome: RemoveRoleOutcome = await roles.remove(
        params.id,
        params.roleId,
        user.id,
        query['cascade'] === 'true',
      ...
      if (!outcome.ok) {
        if (outcome.reason === 'in_use') {
          ...
          // against a project where nothing pointed at this role. The counts
