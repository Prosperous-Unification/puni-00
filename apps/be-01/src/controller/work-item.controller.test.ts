...
function buildHarness(optimized?: OptimizedScheduleReader) {
  ...
  // out again.
  return { register, send, measures: measureStore, projects: projectStore, writes };
}
...
async function setup(optimized?: OptimizedScheduleReader) {
  const { register, send, measures, projects, writes } = buildHarness(optimized);
  ...
  return { token, send, measures, projects, writes, projectId: body.project.id, devId, qaId };
}
