...
const projectPatch = t.Object({
  ...
  // The three optimizer settings (tasks.md 3b.2), each optional and each moving
  // on its own: a project switched off keeps the engine and the objective it
  // was on, which is why they are three columns rather than one nullable
  // engine. They are **project settings**, so they arrive on this route under
  // its existing project-write authorization rather than on a route of their
  // own — a reader may not change them, and `ProjectService.update` is where
  // that is decided for every field alike.
