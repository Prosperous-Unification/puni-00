 ...
 * predecessor's later steps run in parallel with the successor, so an arrow
...
export interface GanttDependencyArrow {
   ...
   * of no days at all — an unestimated first step — is on no workday, and its
...
export function startFloorByRow(
  ...
  for (const row of plan.rows) {
    ...
    try {
      ...
      // a row held by a person, a pool or its own earlier step has a sentence
 ...
 * slice in step order **that somebody estimated**, its last slice when nobody
