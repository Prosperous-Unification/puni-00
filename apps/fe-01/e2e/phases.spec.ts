 ...
 *
 * `phases-panel.test.tsx` proves what the section sends and what it says; this
...
/**
 * Opens the Phases section: the `Project settings` control, then its tab.
 *
 ...
 * rather than about a phases dialog of its own.
 */
async function openPhases(page: Page): Promise<void> {
  ...
  await expect(page.getByLabel('New phase')).toBeVisible();
}
 ...
 * `getByRole('button', { name: 'Phases' })` matched an accessible name by
 ...
 * `Phases`; this stays as the belt beside that.
 */
