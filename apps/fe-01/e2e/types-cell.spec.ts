...
test.describe('the Types cell at a laptop width', () => {
  ...
  test('a row of three types is the same height as a row of none', async ({ page }) => {
    ...
    // the bug.
    //
    // Proof: `flexWrap: 'wrap'` put on `REFERENCE_SET_STRIP_STYLE` — see the
    // run in `work-item-types`' verify.md.
    ...
    // **No strip-height assertion here, and the absence is the finding.**
    //
    // Task 5.1 named `flex-wrap: wrap` as this case's negative. It was injected
    // — on `REFERENCE_SET_STRIP_STYLE` and both chip-group rules — and watched
    // leaving all five cases **green**. `CELL` clips the `<td>`, so a wrapped
    // strip is cut off at the cell rather than pushing the row taller: at the row
    // level wrap and nowrap are indistinguishable, which is the whole point of
    // the clip and also the reason this assertion cannot see it.
    //
    // Measuring the strip instead does not rescue it. The strip is 87px with
    // three chips and 48px with one **under `nowrap`** — it holds the search box
    // as well as the chips and grows with them either way — so a strip
    // comparison fails on correct code. Both attempts are recorded rather than
    // tuned into passing, because a threshold picked to sit between 48 and 87 on
    // this font at this width would be a number reverse-engineered from one run.
    //
    // So the row height above is asserted as what it is: the promise a reader
    // cares about, guaranteed by the clip rather than by `nowrap`. The mechanism
    // is checked directly in the case below, where the fault that *can* be
    // watched — removing the clip — is the one that matters. Deleting the guard
    // whose removal you cannot see, `T1 column-widths-drag`'s own lesson.
  });
  ...
  test('clips the chip run at the cell rather than letting it paint out', async ({ page }) => {
    ...
    // the `<td>` clips, **and** the content is genuinely wider than the cell, so
    // the clip is doing work. Without the second half this passes on a row whose
    // chips fit, which is a check that cannot fail — the fault `G
    // gantt-view` shipped, one file over.
    await addType(page, '010', 'Story');
    await addType(page, '010', 'Spike');
    await addType(page, '010', 'Epic');

    const cell = await page.getByRole('combobox', { name: 'Types for 010' }).evaluate((node) => {
      const td = node.closest('td');
      if (td === null) throw new Error('the strip is not in a cell');
      return {
    ...
    // `clip`, never `hidden`: a hidden box is a scroll container the browser may
    // scroll to reveal a focused chip, which moves the cell's content sideways
    // and leaves the header behind (`table-frame.ts`).
    expect(cell.overflowX).toBe('clip');
    // And the cell is genuinely narrower than what it holds, or the clip above
    // would be a rule with nothing to clip.
    expect(cell.clientWidth, 'the cell was not laid out').toBeGreaterThan(0);
    expect(
      cell.scrollWidth,
