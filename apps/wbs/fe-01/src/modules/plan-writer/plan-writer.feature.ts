...
export function createPlanWriter({
  ...
  return {
    ...
    async run(action) {
      ...
      // Proof: on 2026-09-25, reducing this to `owner !== null` failed `refuses a completed
      // gesture whose feed owner was replaced under it` on `expected 'landed' to be 'refused'`.
      ...
      try {
        ...
        // Proof: on 2026-09-25, removing this check sent a second create in `abandons queued
