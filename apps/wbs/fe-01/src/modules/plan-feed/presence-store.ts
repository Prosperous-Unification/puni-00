...
export function createPresence(): PresenceStore {
  ...
  const replace = (next: Presence): void => {
    // Proof: on 2026-09-24, publishing before this assignment failed `shows exactly the last frame
    // and connection, and changes only when they do` after 1 run on `listener 0: users: expected []
    // to be []`: the listener read the previous snapshot.
    current = next;
  ...
  return {
    ...
    snapshot: () => current,
    reportUsers: (users) => {
      // Proof: on 2026-09-24, dropping this early return failed `shows exactly the last frame and
      // connection, and changes only when they do` after 1 run on `users(["lee"]): a new snapshot
      // exactly when something changed: expected true to be false`.
      // Proof: on 2026-09-24, also returning early while disconnected failed the same test after 1
      // run on `later users([]): a new snapshot exactly when something changed: expected false to
      // be true`.
    ...
    reportConnection: (connected) => {
      ...
      // Proof: on 2026-09-24, replacing with `{ users: [], connected }` here failed `shows exactly
      // the last frame and connection, and changes only when they do` after 1 run on
      // `connection(true): users: expected [] to be []`.
