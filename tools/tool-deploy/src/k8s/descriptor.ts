...
/**
 * What the operator hands staging: the Dagger-published digests (be/gw/fe from
 * `dist/tool-dagger/release.json`, MCP from its own publish) and the CI run that gated the
 * source. It carries no admission identity: those come only from the trusted admission job.
 */
...
/** Validates the operator's candidate at the workflow boundary. */
export function parseReleaseCandidate(input: unknown): ReleaseCandidate {
  ...
  exactKeys(input, ['schemaVersion', 'sourceSha', 'images', 'gateRunId'], 'release candidate');
  ...
  return {
    schemaVersion: 1,
    sourceSha,
    images: parseImages(input['images'], 'release candidate images'),
    gateRunId,
  };
}

/**
 * The gate evidence must be a completed, successful `ci` run on exactly `sourceSha` whose `gate`
 * job and `pixels` browser job both succeeded.
 */
