...
export const OBJECTS = {
  ...
  /** deploy/k8s/wbs/base/verify-cronjob.yaml; same admission, so it moves with the backend too. */
  verifyCronJob: 'sqlite-backup-verify',
} as const;

/** Every CronJob that runs the backend image, with the container carrying it. */
const BACKEND_CRONJOBS = [
  { name: OBJECTS.backupCronJob, container: 'backup' },
  { name: OBJECTS.verifyCronJob, container: 'verify' },
] as const;
...
export function kubectlEffects(settings: KubectlSettings): ReleaseEffects & ClusterReader {
  ...
  /** A backend-image CronJob's image; F6 admits its pods only with a backend digest. */
  async function cronJobImage(name: string, container: string): Promise<string> {
    return (
      await kubectl([
        '-n',
        backend,
        ...
        name,
        '-o',
        `jsonpath={.spec.jobTemplate.spec.template.spec.containers[?(@.name=="${container}")].image}`,
      ])
    ).trim();
  }
  ...
  async function rollout(namespace: string, tier: K8sTier, image: string): Promise<void> {
    ...
    if (tier === 'backend') {
      ...
      // Proof: the same test saw no sqlite-backup-verify patch with it removed from
      // BACKEND_CRONJOBS (2026-09-18).
      for (const cronJob of BACKEND_CRONJOBS) {
        const cronPatch = {
          spec: {
            jobTemplate: {
              spec: { template: { spec: { containers: [{ name: cronJob.container, image }] } } },
            },
          },
        };
        await kubectl([
  ...
  async function currentRelease(): Promise<ReleaseIdentity | null> {
    ...
    for (const cronJob of BACKEND_CRONJOBS) {
      const running = await cronJobImage(cronJob.name, cronJob.container);
      if (running !== identity.images.backend) {
        throw new Error(
          `CronJob ${cronJob.name} runs ${running} but the release record says ` +
