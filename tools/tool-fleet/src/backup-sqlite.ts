...
export async function restoreSqlite(options: {
  ...
  readonly store: ObjectStore;
  /** The report's SHA-256 from the escrowed recovery manifest; omitted only by `verify`. */
  readonly reportSha256?: string;
}): Promise<BackupReport> {
  if (existsSync(options.targetPath)) throw new Error(`${options.targetPath} already exists`);
  const reportBytes = await options.store.get(options.reportKey);
  if (
    options.reportSha256 !== undefined &&
    createHash('sha256').update(reportBytes).digest('hex') !== options.reportSha256
  ) {
    // Proof: with this comparison removed, the replaced-report negative in backup-sqlite.test.ts
    // restored a report the manifest never recorded (2026-09-18).
    throw new Error(`${options.reportKey} differs from the SHA-256 in the recovery manifest`);
  }
  const report = parseReport(reportBytes, options.reportKey);
...
if (import.meta.main) {
  ...
  if (mode === 'backup') {
    ...
    const report = await restoreSqlite({
      ...
      reportSha256: requireEnv('RESTORE_REPORT_SHA256'),
