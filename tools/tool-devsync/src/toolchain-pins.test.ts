...
describe('the CI gate scope', () => {
  ...
  it('keeps Tool Wiki in the pull-request gate, read as JSON rather than grepped', async () => {
    ...
    expect(mode).toContain(`jq -s -e '.[0] | index("twilight-burokrat") != null'`);
  ...
  it('refuses a jq failure instead of reading it as Tool Wiki being unaffected', async () => {
    ...
    expect(mode.indexOf(`jq -s -e 'length == 1 and (.[0] | type == "array")'`)).toBeLessThan(
      mode.indexOf(`jq -s -e '.[0] | index("twilight-burokrat") != null'`),
    );
  });
...
describe('the owner-maintained libraries', () => {
  ...
  it('installs one copy of the report library, the one application-exception loads', () => {
    ...
    // Proof: on 2026-09-25, a copy of the report library nested under application-exception failed
    // this test on the two resolutions, `node_modules/application-exception/node_modules/…` received
    // where the root copy was expected, while the lock-key test beside it passed; with this assertion
    // weakened to a type check the same nested copy passed the whole suite.
    expect(loadedCopy).toBe(rootCopy);
    const { version } = fromRoot(rootCopy) as { version: string };
    // Proof: on 2026-09-25, the installed copy's own `package.json` edited to 12.9.9 failed this test
    // on `Expected: "13.0.0"` / `Received: "12.9.9"`; with this assertion weakened to a type check
    // the same edit passed the whole suite.
