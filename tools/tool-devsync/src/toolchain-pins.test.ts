...
describe('the CI gate scope', () => {
  ...
  it('runs affected on a pull request and the unchanged full gate everywhere else', async () => {
    ...
    expect(occurrences(script, '--exclude=twilight-bureaucrat')).toBe(2);
    expect(
      occurrences(script, 'bunx nx run-many -t test typecheck build -p twilight-bureaucrat'),
    ).toBe(2);
    expect(
      occurrences(
        script,
        'bunx nx run twilight-bureaucrat:lint:source --skip-nx-cache --output-style=stream',
      ),
    ).toBe(2);
  });
  ...
  it('keeps Tool Wiki in the pull-request gate, read as JSON rather than grepped', async () => {
    ...
    // `bunx nx run-many -t test typecheck build -p twilight-bureaucrat` — 2 failed / 14 passed. A
    ...
    // `grep -qx twilight-bureaucrat` over it can never match and would drop the project from every
    ...
    expect(mode).toContain(`jq -s -e '.[0] | index("twilight-bureaucrat") != null'`);
  ...
  it('refuses a jq failure instead of reading it as Tool Wiki being unaffected', async () => {
    ...
    expect(mode.indexOf(`jq -s -e 'length == 1 and (.[0] | type == "array")'`)).toBeLessThan(
      mode.indexOf(`jq -s -e '.[0] | index("twilight-bureaucrat") != null'`),
    );
  });
