...
- The naming tree is fixed by the spec: Prosperous Unification (PUNI) is the company; Vesper Shipyards is the product's code name; Twilight Structure (twist) is the tool suite; Twilight Navigator (twin) plans; Twilight Dash (twid) executes; Twilight Bureaucrat (twib) holds the rules.
...
| Legacy wiki tool names and variables         | 105+  | 1,200+      | Bureaucrat source 307, wiki change 249, bootstrap policy 77, workflows 43 |
| The previous Nx project name of the wiki CLI | 29    | 97          | Dated plans 26, wiki release change 13                                    |
| "Twilight Bureaucrat" and its package name   | 63+   | 438         | Already correct                                                           |
| The short forms twib, twist                  | 0     | 0           | Not used anywhere yet                                                     |

The registry was checked the same day. The package names for Twilight Structure, Twilight Dash, Twilight Navigator and Twilight Bureaucrat are all unclaimed on npm, as is the product code name. The short forms twin, twid and twist are taken as npm package names, so they can only ever be binary names. The short form twib is unclaimed.
...
| The planned Twilight verifier tool                             | Removed from the design. Verification is Twilight Bureaucrat's; compilation is Twilight Dash's.                    |
| Bureaucrat source location under the wiki apps directory       | Stays for now. A physical move is Task 7, optional and late.                                                       |
| Legacy wiki script names, workflow names and variables         | Stay. The Bureaucrat README already fixes their spelling for evidence continuity.                                  |
...
- [x] Redefine Twilight Structure in the glossary and add Prosperous Unification, Vesper Shipyards, Twilight Navigator, Twilight Dash and Twilight Bureaucrat, each with an avoid list.
...
### Task 5: Add the short command name for Twilight Bureaucrat

This task changes a package manifest, so it needs an OpenSpec change named `twilight-bureaucrat-short-command` with intent, a delta spec, tasks and a verification record.
...
### Task 7: Move the Bureaucrat source under its product directory, optional
...
- [ ] Update every declared external consumer in the Bureaucrat README's module index: the lint launcher, the host gate, the trusted workflow, the candidate workflow, the hook file and the Nx configuration.
