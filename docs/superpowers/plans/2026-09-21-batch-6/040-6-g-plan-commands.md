...
unconditionally; it only duplicates the evidence under `preserved/`, which is harmless.
...
- **The announcement collector does not move** (section 3, "Task 1.2"; decided in section 9): it
  is an implementation of the neutral `Broadcaster` port shared by the two admitting features;
  barred from either feature by K6, it stays in `service/broadcast.ts`, imported by Plan commands
  and Plan import.
...
1. `m=libs/wbs/application/core/src/module/plan-commands; mkdir -p "$m"`, create `module.test.ts`
...
11. Append to `openspec/changes/adopt-di-composition/verify.md` a `### Plan commands, Slice 1 — <date>`
    ...
    collector stays in `service/broadcast.ts` as an implementation of the neutral `Broadcaster` port
    shared by the two admitting features, barred from either by K6: Plan import builds one too
    (task 1.2); be-01's
...
1. **Resolved 2026-09-24 (planner decision): the announcement collector is not private to Plan
   commands.** `AnnouncementCollector` is an implementation of the neutral `Broadcaster` port
   shared by the two admitting features; barred from either feature by K6. It stays in
   `service/broadcast.ts`, both Plan commands and Plan import import it, and
   `ports/event-port-boundaries.test.ts` keeps pinning it there. Why not private: a private owner
   would make the other admitting feature import a sibling feature (K6), and a copy would be a
   second class definition, which the map forbids. The backend module map's "Plan commands' private
   collector" was wrong because it did not see Plan import's use. Slice 1 corrects `kinds.json` and
   the contract's K disclosure; slice 4 rewords and ticks 1.2 and 5.2. **Known limit:** the design's
   three-way rule for support that is neither kind (domain library, a repository in disguise, or a
   private member of one module) does not admit a port implementation shared by two features, so
   `kinds.json` still files it as `support`; that residual is owned by the kind rules (addendum
   20), not by this packet. The batch-4 module map (`040-6-backend-module-map.md`) is left as a
   dated plan; the correction lives in `tasks.md`, `kinds.json` and `contract.ts`.
...
5. **The collector beside the port it implements.** The design-consistent end state moves
   `AnnouncementCollector` and `HeldAnnouncement` under `ports/`, beside `ports/project-event.ts`,
   leaving `service/broadcast.ts` a shim; `ports/event-port-boundaries.test.ts:11`'s `collectorHome`
   moves with it.
...
+      "disposition": "an implementation of the neutral Broadcaster port shared by the two admitting features, Plan commands and Plan import; barred from either feature by K6; its end state is beside that port under ports/",
...
+      `service/broadcast.ts` and was to move with 5.2 (superseded below);
+      `module/plan-import/plan-import.feature.ts:148` builds one too, so Plan commands cannot own it
+      privately before that module exists without a K6 feature-to-feature edge.
+      **Decided 2026-09-24, with the Plan commands module landed (5.2): the collector is not Plan
+      commands' private class.** `module/plan-commands/plan-commands.feature.ts` and
...
+      `service/broadcast.ts` as an implementation of the neutral `Broadcaster` port shared by the
+      two admitting features; barred from either feature by K6.
+      `ports/event-port-boundaries.test.ts` keeps pinning it there, and its `kinds.json`
+      disposition says so. Its end state is beside that port under `ports/`, when
+      `collectorHome` moves; the design's three-way rule for support does not admit a port
+      implementation shared by two features, a known limit owned by the kind rules.
...
+- [x] 5.2 Plan commands, with Working plan private to it and the announcement collector left
+      shared (see 1.2). Landed 2026-09-24 as
...
+      test. The announcement collector stays in `service/broadcast.ts`, an implementation of the
+      neutral `Broadcaster` port shared by Plan commands and Plan import: the backend module map's
+      "Plan commands' private collector" did not see Plan import's use, and the task was reworded
+      2026-09-24 (see 1.2).
...
- **The map's "collector private to Plan commands" was wrong.** Both features build
  ...
  Resolved as an implementation of the neutral `Broadcaster` port shared by the two admitting
  features; barred from either feature by K6 (section 9, item 1; its move under `ports/` is item
  5, and the "support" residual a known limit owned by the kind rules); 1.2 and 5.2 are reworded
  and ticked.
...
applied 03
applied 04
tree equals 6e8b1ce2
applied 05
applied 06
applied 07
tree equals 8f6d70e0
applied 08
applied 09
applied 10
tree equals 1a70ed1c
applied 11
applied 12
applied 13
applied 14
...
| 11  | `\|\| test $? -eq 1` after pipelines | used only after one `grep` on `compose.ts`, preceded in the same step 0 by `test -f` on that file (slices 1 and 3); every bundle grep uses the single-command `if … then … else status=$?` form                                                                                                                                                                                                                                                                                                                                                                                 |
...
| 19  | ugrep exits 1 on missing file        | `test -f "$bundle"` precedes every `grep -cF` on a bundle; the step-0 `compose.ts` greps follow `test -f` on `compose.ts` (slices 1 and 3)                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 20  | Promise only what a check keeps      | 5.2 and 1.2 ticked only after their wording records the collector decision (an implementation of the neutral `Broadcaster` port shared by the two admitting features, barred from either by K6; the "support" residual a known limit owned by the kind rules); K2 and the composition hazard disclosed in `contract.ts` and on `CommonServices.commands`; K3/K6 stated as read, not watched; `check-indexes` called index validation; Working plan privacy claimed only as "no module export, no importer outside the module but the barrel's shim" (§3), not as a checked rule |
