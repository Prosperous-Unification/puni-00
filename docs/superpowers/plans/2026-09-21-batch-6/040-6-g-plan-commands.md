...
| Slices     | 1 seals Plan commands (feature, use case, command bindings, module files, three sideways rows), 2 makes the Working plan private to it, 3 installs it once in `composeServices`, 4 registers it in the wiki pilot and records tasks 5.2 and 1.2  |
| Implements | `openspec/changes/adopt-di-composition/tasks.md` task 5.2 (recorded, **not ticked**: section 3's measurement of the collector), the remaining half of task 1.2 (measured, **not closable as written**), and task 7.5 for `module/plan-commands/` |
...
real base in place of `50720e10` and E8's real commits present): three diffs here carry dated E8 lines as
context (10.17's second block and 10.18, both on E8's legacy-pin `Proof:` line, and 10.19 on E8's
`tasks.md` note), and if E8's executor observed a date other than 2026-09-24 those lines differ. A refusal there is re-cut by the
...
- **The announcement collector does not move** (section 3, "Task 1.2"): Plan import builds one too,
  so it cannot be private to Plan commands. Its `kinds.json` disposition is corrected; tasks 1.2 and
  5.2 stay unticked with dated notes.
...
| `openspec/changes/adopt-di-composition/tasks.md`                                                | 4          | 1.2 and 5.2 notes (not ticked), 7.5 extended (10.19)                                                             |
...
**The per-scope fault** (row 26). In `compose.ts`, the line `      batchServices: batch,` directly
after `    commands: installPlanCommands({` (not the one inside `installPlanImport({`) is
**replaced** by the two lines

```text
      batchServices: (_scope, broadcast) =>
        servicesOver(source.stores, { clock: runtime.clock, broadcast, scheduler: runtime.scheduler }),
```
...
| 25  | **evidence**: row 26 on the whole file            | the per-scope fault                       | the whole `compose.test.ts`                                                                                  | `13 pass`, `4 fail` (the three fixture-runner cases and the new one), each timing out                                                                                                                                                                                    |
| 26  | `compose.ts`                                      | the per-scope fault                       | `-t "installs Plan commands once"`                                                                           | `this test timed out after 5000ms.`; `0 pass`, `16 filtered out`, `1 fail`; typecheck exit 0                                                                                                                                                                             |
...
**Why the per-scope red is a timeout.** A batch built over the process's own stores asks for the
source's turn while its own unit of work holds it, so it waits until Bun's 5-second test timeout
(the moved `plan-commands.ts` records the same symptom for a lock held across a publish). The
assertion after it never runs; the fault is still watched failing, and only this case is run.
...
11. Append to `openspec/changes/adopt-di-composition/verify.md` a `### Plan commands, Slice 1 — <date>`
    ...
    collector stays in `service/broadcast.ts`: Plan import builds one too (task 1.2); be-01's
...
5. Row 26, restored and `cmp`-proved, against
   ...
   (run with `-t`: row 25 shows why), and `wbs-core:typecheck` on the mutated tree → exit 0. Expect
   the run to take about six seconds.
...
### Slice 4 — Register Plan commands in the wiki content-review pilot and record tasks 5.2 and 1.2
...
6. Apply 10.19 (`tasks.md`: 1.2's and 5.2's dated notes, **neither ticked**; 7.5's running note
   extended).
...
Planner commit: `docs(core): register the Plan commands module in the wiki content-review pilot`.
...
1. **A decision on the announcement collector (task 1.2).** Either an owner both admitting features
   may import (a neutral application-support location, with `ports/event-port-boundaries.test.ts`'s
   `collectorHome` moved with it), or 1.2 and 5.2 reworded to "shared per-admission support". Either
   is a one-slice packet once decided; 5.2 is then tickable.
...
+      "disposition": "shared per-admission announcement collector; stays until task 1.2 gives it an owner both admitting features may import",
+      "rationale": "the Plan commands and Plan import features each build one AnnouncementCollector per admitted act, so it cannot be private to either without a feature-to-feature edge, and the event contracts it re-exports for compatibility now live in ports/project-event.ts"
     },
     {
...
+    runner: graph.commands,
     ...
     runtime,
     pushed,
@@ -234,6 +228,30 @@ describe('composeServices', () => {
...
--- a/libs/wbs/application/core/src/compose.test.ts
+++ b/libs/wbs/application/core/src/compose.test.ts
@@ -249,6 +249,9 @@ describe('composeServices', () => {
     ]);
...
- **The map's "collector private to Plan commands" cannot hold while Plan import collects.** Both
  features build `AnnouncementCollector` per admitted act; private ownership by either is a K6 edge
  for the other. Tasks 1.2 and 5.2 stay open on that one point, with the measurement in their notes
  and the collector's `kinds.json` disposition corrected (section 9, item 1).
...
applied 03
applied 04
tree equals 1b90232f
applied 05
applied 06
applied 07
tree equals 767a519b
applied 08
applied 09
applied 10
tree equals 28bc50f8
applied 11
applied 12
applied 13
applied 14
applied 15
applied 16
tree equals fdd74667
all 16 diffs applied in slice order; every slice tree equals its rehearsal commit
```

The rehearsal commits are throwaway: slice 1 `1b90232f`, slice 2 `767a519b`, slice 3 `28bc50f8`,
slice 4 `fdd74667`, on branch `rehearse/040-6-g-plan-commands` above `50720e10` (not pushed; kept
...
| 15  | Interleaving property tests          | not triggered: no owner, queue, lock or retry logic is added or changed; the per-batch collector and per-scope graph are construction facts, each proved by one substitution fault (rows 8 and 26)                                                                                                                                                                           |
