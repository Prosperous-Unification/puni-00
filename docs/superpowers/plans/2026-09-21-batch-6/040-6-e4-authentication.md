...
| `docs/code-organization/kinds.json`                                                                                            | Both rows for `service/auth.service.ts` and `service/login-throttle.ts` are already classified (`feature`/`authentication` and `support`/"move to the authentication module") — slice 2 rewrites their disposition, not their classification.                                                                                                                                                                                                 |
...
| 7   | `contract.ts`'s own `account.users` intersection member                                                            | `NonNullable<AuthServiceOptions['identities']>` removed from the `users` type (leaving only `AuthServiceOptions['users']`)                                                                                                                                   | the compile-negative test's own `@ts-expect-error` (`verifierWithoutStore`)                                                                                 | `wbs-core:typecheck` failed with `TS2578: Unused '@ts-expect-error' directive` — the insufficient `UserStore`-only store then satisfied the (weakened) requirement                                                                                                                                                                      |
...
| 17  | slice 3, legacy-pin, on the unchanged pin literal                                                                  | none; the registration edit alone moves the count                                                                                                                                                                                                            | `every legacy source occurrence and relevant text family is pinned`                                                                                         | `historical policy selector or baseline`: `Expected: 45`, `Received: 47`; `occurrences`: `Expected: 263`, `Received: 265`; digest changed to `3eca3cf1a2f8d1703b42edfd40be279a5a144034c000a812d7b70eb8b2cfef62`; `unclassified: []`                                                                                                     |
...
6. Apply section 10.5's diff to `ports/sideways-type-boundaries.test.ts`: add five new rows. This
   is a pure addition, not a rename: no existing row ever targeted a single Authentication file.
...
7. Rehearse the negatives, **each restored and `cmp`-proved before the next**:
   ...
   - Row 7: remove `NonNullable<AuthServiceOptions['identities']>` from `contract.ts`'s own
     `account.users` intersection, observe `wbs-core:typecheck` fail with exactly
     `TS2578: Unused '@ts-expect-error' directive`; restore, `cmp`-prove, rerun typecheck green.
   ...
   Add the dated `Proof:` comments only now, after observation: two beside `module.ts`'s
   `buildModule` call (rows 3 and 3b — the widened-tuple fault; the label-drop fault gets its own
   comment too), two beside `check.ts`'s `return` (rows 5 and 6), one beside `contract.ts`'s own
   `account` type (row 7), and five in `sideways-type-boundaries.test.ts`'s own JSDoc above
   `routes` (rows 8 through 12, each naming its own fault and fragment separately).
...
Planner commit: `refactor(core): seal Authentication as a DI Bag module`. The planner's own `git mv`
records the rename Git itself detects for `login-throttle.test.ts`; the two production files are
...
baseline the way slice 1 does and call it `C` (observed `570 pass` over 59 files after slice 1).
...
Now, and only now, add the dated `Proof:` comment to `compose.test.ts`'s own new negative, naming
what was actually observed in step 1.

Tick nothing in `tasks.md` here either. Append to `openspec/changes/adopt-di-composition/verify.md`:
`base`, `K`, `C`, the two step-1 reds with their literal diagnostics, the portable-bundle grep
result, the be-01-typecheck baseline (pass/fail only), and the closing counts. Only after those
appends: `GSETTINGS_BACKEND=memory bunx nx format:check --all` → exit 0.
...
6. Rerun the whole `pilot-policy.test.ts` file → expect GREEN: `21` tests, `0` failures, `297`
   `expect()` calls — one more than the step-0 baseline `P` (the `.every()` membership check now
   runs over ten modules instead of nine, never an absolute figure).
7. Apply section 10.10's diff to `tools/tool-devsync/src/repo-namespacing-handoff.test.ts` (the
   legacy-pin re-pin). Rerun the legacy-pin test alone → expect `1 pass`. This is the ONLY
   permitted edit to a pinned literal in that file; report a stop if any OTHER pinned count in it
   also moved.
 ...
 * directive" — the insufficient store then satisfied the requirement.
 */
 ...
 * module's own `check.ts` never leaves it unset: it always derives
 * `identities` from `account.users`, so a host of this module cannot supply
...
Task 7.5's own running note gains one further paragraph after Realtime's own entry (before Plan
import's own "Not landed" paragraph), recording the registration and explicitly withdrawing the
prior revision's impossibility finding — see section 13's own exact wording, landed verbatim.
