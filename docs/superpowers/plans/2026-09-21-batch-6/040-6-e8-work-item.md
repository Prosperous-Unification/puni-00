...
- No change to `service/service-boundaries.test.ts` (its list decides when a shim may go, task 7.1), to be-01's
...
| **`compose.test.ts`'s private reads.** `runtimeOf` (`compose.test.ts:125-139`) casts `services.workItems` to read the private `clock` field and `opts.scheduler`, `opts.broadcast`. The installer builds `new WorkItemService(workItemOptions)`, whose `opts` is the module's options object and whose `clock` is `opts.clock`, so `runtimeOf` itself needs **no edit**. Its one caller, `shares runtime identities while creating a fresh scope, collector and graph per batch` (`:362-365`), compares the three fields with `toEqual`, which proves member equality, not the identity its title names: a structurally equal copy `{ ...scheduler }` handed from the module's factory passed it (row 16). Slice 2 is the change that routes these three values through DI Bag factories, so it rewrites that assertion as per-field `toBe` (10.8), green on the unchanged tree and red on the same copy (row 15).                                                                                                                                                                                                                                                                                                            | Read; rehearsed.                                      |
...
edge fault edits only the one directly after `        journal: journalStore,`. **Replace** that
line; do not add a second `broadcast` key beside it: a later shorthand `broadcast,` wins in the
object literal and the named test stays green (observed in review: `5 pass`).
...
**The runtime fault** (row 15). In `module.ts`, in the returned object, the line
`        scheduler,` directly before `        clock,` / `      }),` (the returned object's, not the
parameter list's, which is followed by `        clock,` / `      }: {`) is **replaced** by
`        scheduler: { ...scheduler },`.
...
| 15  | `module.ts`, the `workItemOptions` factory's return, after 10.8    | the runtime fault                                                                                                                   | `-t "shares runtime identities"`                                                                         | `error: expect(received).toBe(expected)` … `Received: serializes to the same string`; `0 pass`, `15 filtered out`, `1 fail`; typecheck exit 0                                                                                   |
| 16  | **evidence**: row 15 before 10.8, on the `toEqual` assertion       | the runtime fault                                                                                                                   | same test                                                                                                | `1 pass`, `0 fail` — the false green 10.8 closes                                                                                                                                                                                |
...
interface, so a swapped store does not compile — and by row 15, through the rewritten identity
assertion, for the scheduler (the same assertion holds the clock and the broadcaster).
...
3. Apply 10.8's diff (one `compose.test.ts` case, and `shares runtime identities while creating a
fresh scope, collector and graph per batch`'s `toEqual` over `graphs.map(runtimeOf)` rewritten
   as per-field `toBe`; `runtimeOf` itself is unchanged). Rerun `compose.test.ts` → row 13
   (`16 pass`, `0 fail`).
...
5. The negatives, one at a time, each restored and `cmp`-proved: row 14 against
   ...
   not required.
6. Only now apply 10.9's diff (the two Proof comments: the new case's and the rewritten identity
   assertion's).
...
9. Append `### Work item installation, Slice 2 — <date>` to `verify.md`: `base`, `K`, `C`/`F`,
   ...
   and the line: "`shares runtime identities while creating a fresh scope, collector and graph per
batch` now asserts the installed Work item's clock, scheduler and broadcaster by identity
   (`toBe`), watched failing on a structurally equal scheduler copy; no `servicesOver` resource is
...
### 10.8 The Work item per-scope case and the identity assertion (slice 2 step 3)
...
@@ -359,10 +359,13 @@ describe('composeServices', () => {
...
@@ -491,4 +494,17 @@ describe('servicesOver', () => {
...
### 10.9 Slice 2's Proof comments (slice 2 step 6 — only after rows 14 and 15 were observed)

```diff
diff --git a/libs/wbs/application/core/src/compose.test.ts b/libs/wbs/application/core/src/compose.test.ts
--- a/libs/wbs/application/core/src/compose.test.ts
+++ b/libs/wbs/application/core/src/compose.test.ts
@@ -359,6 +359,10 @@ describe('composeServices', () => {
     expect(graphs[0]).not.toBe(graphs[1]);
     expect(graph.clock).toBe(clock);
     expect(graph.scheduler).toBe(runtime.scheduler);
...
+    // passed the same copy.
     ...
     for (const [index, services] of graphs.entries()) {
       const seen = runtimeOf(services);
@@ -505,6 +509,9 @@ describe('servicesOver', () => {
         name: 'Scope',
       }),
     ).toMatchObject({ ok: true });
...
- **A test named for identity proved only equality.** `shares runtime identities while creating
a fresh scope, collector and graph per batch` asserted the private runtime fields with `toEqual`,
  so a structurally equal copy passed (row 16). Slice 2 closes it with per-field `toBe` (row 15).
...
applied 02
applied 03
tree equals dc3bd654
applied 04
applied 05
applied 06
tree equals 6460e975
applied 07
applied 08
applied 09
applied 10
applied 11
applied 12
tree equals 40333be3
all 12 diffs applied in slice order; every slice tree equals its rehearsal commit
```

The rehearsal commits are throwaway: slice 1 `dc3bd654`, slice 2 `6460e975`, slice 3 `40333be3`, on
...
| 20  | Promise only what a check keeps      | 5.1 ticked only because no `new` resource remains (slice 2 step 8's grep); K2 disclosed, K4/K6 stated as read, not watched; `runtimeOf`'s value comparison closed with `toBe` and watched failing (row 15); `check-indexes` called index validation; no compile red claimed (§6) |
