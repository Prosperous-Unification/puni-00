...
| **A2** | make the post-disposal fence always fire while keeping it a condition, so the code below it still narrows: `if (ordinal !== newest \|\| newest >= 0) throw new TransitionSupersededError(ordinal, newest);` (a bare unconditional `throw` makes TypeScript stop narrowing `request` below it and the mutant does not compile: TS2339, TS18046; found on the first slice 3 attempt, 2026-09-22) | `25 failed (25)`. Model test: `AssertionError: the latest request did not win: expected 'refused' not to be 'refused'`, counterexample `[{replace,settles,none},{settle}]`, `Shrunk 3 time(s)`                                                  |
