...
| Packet | Reason                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 060.1  | Held on 2026-09-19 for the two reasons already stated: it needs a container daemon, whose authority no working directory confines, and its experiment did not yet prove persistence on one immutable image. Released on 2026-09-20 for a split run — a sandboxed executor with network access prepares research, scripts and Dockerfiles, and the planner runs every container step on the workstation from the reviewed scripts — after the owner decided that running software written for Node on Node is fine. See the [dispatch amendment](060-1-evaluate-emdash.md#dispatch-amendment-2026-09-20-two-actors-one-preparation-checkpoint). |
...
**Frontend tests inside the sandbox.** Three frontend tests in two files, `apps/wbs/fe-01/playwright-config.test.ts` and `apps/wbs/fe-01/src/components/wbs/short-date.test.ts`, spawn `bun` from Node, and the executor's sandbox refuses that with `spawnSync bun EPERM`. So `wbs-fe-01:test:unit` and `wbs-fe-01:test` cannot pass inside an attempt, though both pass outside it (observed 2026-09-20). An executor never runs those two targets. Where a packet wants the unit tier as a broad net, it runs the sandbox unit command, `(cd apps/wbs/fe-01 && bunx vitest run --config vitest.node.config.ts --exclude playwright-config.test.ts --exclude src/components/wbs/short-date.test.ts)`, and compares it only with its own earlier run of the same command. The planner runs both whole targets outside the sandbox against the recorded baseline. Nothing in the repository is excluded or skipped; the exclusion lives only on the executor's command line.
