# Twilight Burokrat Package and CI/CD Implementation Plan

> **For agentic workers:** Use `executing-plans`; execute P0–P5 in order with a commit and observed verification per task.

**Goal:** Install and run `twilight-burokrat` from a pinned npm release in both ordinary CI and trusted admission.
...
**Spec:** [B1–B3 in the delivery contract](../specs/2026-09-17-twilight-burokrat-and-fleet-design.md).

## Global constraints

All [master-plan constraints](2026-09-17-twilight-burokrat-and-fleet.md#global-constraints) apply. Preserve version-1 evidence and activation semantics. Published artifacts must not require the source checkout. Commands below are implementation targets unless explicitly identified as existing.
...
- [ ] Verify available registry namespace with `bun info twilight-burokrat`; distinguish not-found from authentication/network failure. Record registry access as pending if unavailable. Do not choose a different name without the user.
...
**Create:** `apps/twilight-burokrat/cli/package.json`, `src/bin.ts`, `src/bin.test.ts`, `src/packaging/build.ts`, `src/packaging/build.test.ts`.
...
- [ ] Run `bunx nx run twilight-burokrat:test --skip-nx-cache`, `:lint:source`, `:typecheck`, `:build`. Commit after all relevant tests pass.
...
**Files:** rename `.github/workflows/wiki-release.yml` to `twilight-burokrat-release.yml`; update moved release tests, `apps/twilight-burokrat/consumer/README.md`, package release documentation.
...
- [ ] Trigger only `twilight-burokrat-v*`, resolve tag to commit, verify manifest version, run uncached source checks and P3, then upload the verified tarball between jobs. The publishing job downloads it, verifies its digest and publishes that file. It never rebuilds after verification.
...
- [ ] Publish with `bun publish <tarball> --access public`, then install `twilight-burokrat@0.1.0` into a fresh consumer and compare registry integrity with the tested tarball. Record license/ownership check, version, source SHA, registry URL and test evidence. If credentials are unavailable, leave this single publication step visibly pending with the tested tarball ready.
...
**Files:** root `package.json`, `bun.lock`, `.github/workflows/{ci,trusted-wiki}.yml`, `lefthook.yml`, `bin/tool-wiki-{lint,push-audit}.sh`, `bin/h2puni-gate{,-lib,-steps}.sh`, `apps/twilight-burokrat/consumer/trusted-wiki.yml`, `docs/runbook-tool-wiki-activation.md`.
**Create:** `infra/ci/burokrat/{package.json,bun.lock}`, `tools/tool-devsync/src/burokrat-consumer.test.ts`.
...
- [ ] Rewrite source-project exclusions in the full gate from `wiki-cli` to `twilight-burokrat`, retain source lint/test/typecheck for the producer, and run the installed package once for consumer admission. Keep `tool-workflows:integration` with its pinned OpenSpec CLI.
