...
|                                               |                                                                                                                                                                                                                          |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Work item                                     | 110.1, parent 110 "Also needed". Note on the item: "Feeds the evaluations in the three Twilight Burokrat runs."                                                                                                          |
...
into Twilight Burokrat. No file is added under `apps/wiki/cli`, so the
...
| ------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `docs/wiki-policy/relationships*.json`, seven `nx-target` facts           | `wbs-core:test`, `wbs-domain:test`, `wbs-store-memory:test`, `tool-dagger:test`, `twilight-burokrat:{test,lint:source,typecheck}` | None is edited. With slice B's `nx.json` entry applied, `committed-target-facts.test.ts` still passed 2 of 2.                                                                                 |
...
- [ ] B1 Add a `test:api` target default to `nx.json`. Read `targetDefaults` first: if it already holds entries such as `test:store` or `test:portable` carrying `options.env` with `CLAUDECODE` and `AGENT`, copy that entry's exact shape under the key `test:api`. If it holds none — which is the state of this worktree — use:
  ...
  The incoming fix branch adds a graph-walking case that refuses a test-running target name without such a default, so a new target name must bring one. Verified on 2026-09-20: with this entry, `nx show project wbs-store-sqlite --json` resolves `test:api` carrying both variables and `cache: false`, and the target-facts comparison still passed 2 of 2 (that comparison has since moved to `committed-target-facts.test.ts` in the Twilight Burokrat package, which reads targets as its static extractor does: a target with its own `options` does not show a default's `options.env` there, so never add `env` to an `nx-target` fact).
