# Burokrat JSON Declaration Omission Implementation Plan
...
**Spec:** `openspec/changes/twilight-burokrat-kind-rules/specs/burokrat-rules/spec.md`; accepted scope in `docs/superpowers/plans/2026-09-21-batch-4/adoption-tail-map.md` at `f1392c01`.
...
| ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `openspec/changes/twilight-burokrat-kind-rules/specs/burokrat-rules/spec.md` | State complete non-JSON declaration coverage, real JSON graph edges and explicit refusal for an unsupported JSON public surface.                                                               |
| `openspec/changes/twilight-burokrat-kind-rules/tasks.md`                     | Add one unchecked next-numbered task, then check it only after all proofs and final validation. Preserve every pre-existing task and its current checked state exactly.                        |
| `openspec/changes/twilight-burokrat-kind-rules/verify.md`                    | Record immutable commits, reds, mutations, restores, greens, exact commands and limitations.                                                                                                   |
...
2. Synthesizing a `.d.ts` from JSON would make Burokrat a declaration generator and risks disagreeing with compiler inference.
...
- Modify: `openspec/changes/twilight-burokrat-kind-rules/specs/burokrat-rules/spec.md`
- Modify: `openspec/changes/twilight-burokrat-kind-rules/tasks.md`
- Modify: `openspec/changes/twilight-burokrat-kind-rules/verify.md`
...
- **WHEN** Twilight Burokrat extracts relationships
...
- **WHEN** Twilight Burokrat constructs the public declaration closure
...
```bash
OPENSPEC_TELEMETRY=0 bunx @fission-ai/openspec@1.12.0 validate twilight-burokrat-kind-rules --strict --json
```

Expected: exit 0; exactly one item, id `twilight-burokrat-kind-rules`, `valid: true`, no issues. Parse and assert those fields with `jq`; an exit code alone is insufficient.

- [ ] **Step 5: Commit the artifact amendment.**

```bash
git add openspec/changes/twilight-burokrat-kind-rules/specs/burokrat-rules/spec.md \
  openspec/changes/twilight-burokrat-kind-rules/tasks.md \
  openspec/changes/twilight-burokrat-kind-rules/verify.md
git commit -m 'spec(burokrat): define JSON declaration omission'
```
...
git commit -m 'fix(burokrat): admit JSON declaration omission'
```
...
git add apps/wiki/cli/src/relationships/typescript.ts \
  apps/wiki/cli/src/relationships/relationships.test.ts
git commit -m 'test(burokrat): prove JSON declaration boundaries'
```
...
NX_DAEMON=false bunx nx run twilight-burokrat:test --skip-nx-cache
NX_DAEMON=false bunx nx run twilight-burokrat:build --skip-nx-cache
NX_DAEMON=false bunx nx run twilight-burokrat:test:package --skip-nx-cache
OPENSPEC_TELEMETRY=0 bunx @fission-ai/openspec@1.12.0 validate twilight-burokrat-kind-rules --strict --json
GSETTINGS_BACKEND=memory NX_DAEMON=false bunx nx format:check --all
```

Record exact exits and test counts. The whole project test sets `TOOL_WIKI_TRUSTED_NODE_MODULES`; do not substitute a bare root `bun test`. `twilight-burokrat:test:package` is required because the bundled standalone compiler path is load-bearing. `tool-devsync:test` remains outside this five-file slice; do not run it and record it as skipped by scope. Do not run raw full Nx gates.
...
```bash
git add openspec/changes/twilight-burokrat-kind-rules/tasks.md \
  openspec/changes/twilight-burokrat-kind-rules/verify.md
git commit -m 'docs(burokrat): close JSON declaration prerequisite'
```
