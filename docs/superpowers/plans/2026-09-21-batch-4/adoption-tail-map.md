...
- Accepted scope and ordering: `docs/superpowers/plans/2026-09-17-personal-package-adoption.md`,
  ...
  `docs/superpowers/specs/2026-09-19-twilight-burokrat-rules-design.md` B2/B4.
...
| ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `twilight-burokrat` validator      | `apps/wiki/cli/src/cli.ts:runCli`; admission store opened by `writeAdmissionSubmission`                                | Root-local reporting/typed operational failures; preserve every JSON/text stdout and exit. The store remains owned by the command and closed in `finally`.                                                                                                                                                                                  |
...
## Burokrat prerequisites before WBS items 090 and 100

The pending WBS update names `7ad9e8a6…` “Run Twilight Burokrat … WBS backend” and
...
| 2   | Fresh compiler probes reproduce declaration-only `emitSkipped` with no diagnostics both with and without `infra/local/vm-lab.ts`. Per-source emission succeeds for all 38 TypeScript sources; only imported `infra/fleet/schemas/operation-plan.json` is intentionally skipped. | Correct declaration extraction to distinguish compiler-supported JSON omission from genuine declaration failure, preserving complete TypeScript declaration coverage, JSON dependency/public-surface identities and fail-closed behavior. Prove the tools scope through the production CLI; retain the wrapper include and typecheck ownership. | Routine diagnosis corrects the earlier unconfirmed external-include hypothesis; no user choice. Never accept arbitrary `emitSkipped` merely because diagnostics are empty. |
...
| 6   | Accepted survey: F7 at 400 lines reports 107 files; current policy still has no selected ceiling/adopted set. Source may have moved, so recount after selecting the ceiling.                                                                                                    | User selects the ceiling; implementation then records `sizeCeilings` and the exact pinned/adopted debt in the repository rule policy, proves growth/new-overage/removable-pin negatives, and moves the temporary devsync ratchet to Burokrat per rollout Task 9.                                                                                | **Only unresolved user decision:** the numeric F7 ceiling. The adopted set is mechanically generated/reviewed at that ceiling, not a second preference.                    |

A fresh read-only compiler probe on integration `0a399fdc15cf2a19b8b1bdf8c468f5e4abea5815`
corrects prerequisite 2's original survey hypothesis. The installed alias delegates to
`@typescript/old` and exports `ts.version` 6.0.3. The original root set emitted 38 outputs;
removing only `vm-lab.ts` emitted 37, but both returned `emitSkipped: true` with no diagnostics.
A per-source run emitted declarations successfully for all 38 TypeScript sources, including
`vm-lab.ts`; only `infra/fleet/schemas/operation-plan.json` returned skipped with zero outputs.
The installed compiler's `getOutputPathsFor` intentionally provides no JSON declaration path,
and `emitDeclarationFileOrBundle` marks that declaration-only omission as skipped. Evidence is
retained in `fleet-declaration-probe.{ts,jsonl,stderr}` and
`fleet-declaration-source-probe.{ts,json,stderr}` under
`/tmp/puni-codex-resume-20260921/`. Both Bun probes exited 0 and wrote no emitted project files.
These are compiler API observations, not production CLI or whole-project gate results; the
future slice still requires a minimal production fixture, completeness/failure negatives, and
an immutable tools-scope run. No project configuration was changed by the investigation.

Prerequisites 1–4 and 6 correct the existing B2 production path owned by
`openspec/changes/twilight-burokrat-kind-rules`: its checked tasks already implement adopted-set/F7,
...
it adds declarations without changing rule behavior. Every Burokrat B slice still owes intent, delta
...
4. On the independent Burokrat branch, scope prerequisites 1–4 and 6 through the existing B2 artifact
...
authentication for the Burokrat/WBS branch, and the runtime segment grammar plus later-collision rule
