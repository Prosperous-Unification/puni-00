# Burokrat Ambient Non-Code Imports Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development
> (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use
> checkbox (`- [ ]`) syntax for tracking.

**Goal:** Let Twilight Burokrat evaluate a TypeScript candidate whose compiler accepts an ambient
...
`ts.version` string as `6.0.3`); ArkType records; Twilight Burokrat CLI; OpenSpec 1.12.0. The immutable
gate must record its own runtime versions rather than inheriting these planning-host observations.

**Spec:** `openspec/changes/twilight-burokrat-kind-rules/specs/burokrat-rules/spec.md`, requirement
“Kind direction over the import graph,” amended by Task 1 below.

## Global Constraints

- Work under the existing `twilight-burokrat-kind-rules` B2 change; do not create a second change or
  reopen ADR 0029.
...
| ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `openspec/changes/twilight-burokrat-kind-rules/specs/burokrat-rules/spec.md` | Add the missing observable ambient-import contract and fail-closed scenario.                                                                           |
| `openspec/changes/twilight-burokrat-kind-rules/tasks.md`                     | Append one unchecked Part F TDD task before implementation; check it only after evidence exists.                                                       |
| `openspec/changes/twilight-burokrat-kind-rules/verify.md`                    | Record actual RED/mutation/GREEN commands and outputs after execution; never pre-fill proposed observations as facts.                                  |
...
- **WHEN** Twilight Burokrat extracts TypeScript relationships
...
```sh
bunx @fission-ai/openspec@1.12.0 validate twilight-burokrat-kind-rules --strict --json
```
...
git commit -m "spec(burokrat): define ambient import relationships"
```
...
git add apps/wiki/cli/src/relationships/typescript.ts \
  apps/wiki/cli/src/relationships/relationships.test.ts
git commit -m "fix(burokrat): admit ambient non-code imports"
```
...
git commit -m "test(burokrat): keep ambient graph failures unevaluated"
```
...
NX_DAEMON=false bunx nx run twilight-burokrat:lint:source
...
git commit -m "docs(burokrat): verify ambient import extraction"
```
...
- **Scope:** no frontend change, asset existence rule, extension list, schema field, new capability,
  package release, or Burokrat prerequisite 2–6 enters this packet.
