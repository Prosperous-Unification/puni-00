...
| Discover                   | `bunx nx run tool-fleet:discover -- --fleet <fleet.yaml> --output <new observation.json>`                                                                                            | `tool-fleet:build` controller image; `HCLOUD_TOKEN`, `KUBECONFIG`, SSH agent; `--lab-state <dir>` instead for a VM lab               | F12: refused without `--fleet`. Evidence: VM lab only; production discovery has never run                                                                                                                                             |
