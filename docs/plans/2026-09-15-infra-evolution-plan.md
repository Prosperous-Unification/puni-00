...
- Products: WBS (prod `wbs.bulletpoints.club`, dev `dev.wbs.bulletpoints.club`),
  ...
  on h3, the claire harness on h1, and Twilight, whose ADR 0028 commits to k3s
...
| Twilight worker cluster      | separate k3s cluster; M1 joins CPX32 agents by hand per ADR 0028; the cluster-autoscaler hetzner provider is the later Terragrunt scope | 41.99 per node, hourly | ADR 0028 M1                                                                 |                                                                                                                                                                                                                                                        |
...
- Costs restated as gross and net per stage; stage 3 itemized; Twilight row
  aligned with ADR 0028 (manual join at M1); CloudNativePG needs three nodes;
