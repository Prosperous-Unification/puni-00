...
| `docs/2026-09-02-refactoring-plan.md`, `docs/2026-09-05-ports-and-adapters-plan.md`, [Radical Modularity](docs/plans/2026-09-08-agent-scalable-llm-wiki.md), [precedents and extraction](docs/plans/2026-09-13-tool-wiki-precedents-and-extraction.md), `tools/harness-example/src/harness.ts` | before refactoring or designing model-scale work: ports split complete and ADR 0014–0015 accepted; adjustable boundaries and the LLM wiki remain queued. Refactoring `-history.md` is not normative. **Status and queue: `docs/refactoring/tasks.md`**. The last is the smallest runnable LangGraph harness: plan → parallel implement → revise → finish, one ACP agent session per task, ArkType state, Claude Code over ACP for real or an in-process fake; `bunx nx run harness-example:run -- --fake x` |
