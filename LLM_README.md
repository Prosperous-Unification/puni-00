...
- **Capacity C2 must not be deployed without C3.** #53 lets a client size a team, be-01 then emits
  `boundBy: 'capacity'`, and fe-01's `floorWordsOf` throws on it by design. Merging is safe,
  deploying is the gate. **#57 (`capacity-ui`) closes it** — delete this on merge, not before.
