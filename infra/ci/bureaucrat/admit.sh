...
refuse() {
  printf 'twilight-bureaucrat admission: %s\n' "$1" >&2
  exit 78
}
...
if ! identity=$(
  env -i PATH="$trusted_path" HOME="$scratch/home" CONSUMER="$scratch/consumer" \
    ...
    const pin = (await read(`${process.env.CONSUMER}/package.json`)).dependencies["twilight-bureaucrat"];
    ...
    if (typeof pin !== "string" || installed.name !== "twilight-bureaucrat" ||
    ...
    process.stdout.write(`twilight-bureaucrat-v${pin} ${packaged.toolkitIdentity}`);
  '
); then
...
env -i PATH="$trusted_path" HOME="$scratch/home" CONSUMER="$scratch/consumer" \
  ...
  const record = {
    ...
    package: {
      ...
      integrity: lock.packages["twilight-bureaucrat"][3],
