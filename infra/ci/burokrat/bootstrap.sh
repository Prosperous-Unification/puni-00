...
# Installs the base-owned Twilight Burokrat pin into a fresh runner scratch directory, before any
...
refuse() {
  printf 'twilight-burokrat bootstrap: %s\n' "$1" >&2
  exit 78
}
...
if ! pin=$(
  trusted_bun MANIFEST="$manifest" "$bun_path" --cwd "$scratch/home" --no-env-file -e '
    ...
    if (JSON.stringify(keys) !== JSON.stringify(["dependencies", "name", "private"]) ||
        ...
        JSON.stringify(Object.keys(dependencies)) !== JSON.stringify(["twilight-burokrat"]) ||
        typeof dependencies["twilight-burokrat"] !== "string" ||
        !/^[0-9]+\.[0-9]+\.[0-9]+$/.test(dependencies["twilight-burokrat"])) process.exit(1);
    process.stdout.write(dependencies["twilight-burokrat"]);
  '
); then
  refuse "base-owned bootstrap manifest must pin exactly twilight-burokrat and nothing else"
fi
...
if ! integrity=$(
  trusted_bun LOCK="$lock" PIN="$pin" REGISTRY="$registry" "$bun_path" --cwd "$scratch/home" \
    ...
    const entry = lock.packages?.["twilight-burokrat"];
    if (JSON.stringify(Object.keys(workspaces ?? {})) !== JSON.stringify([""]) ||
        ...
        JSON.stringify(root.dependencies) !== JSON.stringify({ "twilight-burokrat": pin }) ||
        JSON.stringify(Object.keys(lock.packages)) !== JSON.stringify(["twilight-burokrat"]) ||
        !Array.isArray(entry) || entry[0] !== `twilight-burokrat@${pin}` ||
  ...
  refuse "base-owned bootstrap lock does not pin twilight-burokrat@$pin from $registry"
fi
...
if ! (cd -- "$scratch/consumer" &&
  ...
  refuse "frozen install of twilight-burokrat@$pin failed"
fi
...
printf 'package=twilight-burokrat@%s\n' "$pin"
printf 'integrity=%s\n' "$integrity"
