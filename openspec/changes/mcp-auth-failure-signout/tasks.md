...
- [x] 2.2 `wbs-client.ts`: typed `UpstreamRejected` vs `EdgeGate`, where a Basic challenge is `EdgeGate` even if missing `WBS_BASIC_AUTH` caused the rejection.
  - Proof: treating any challenge as edge makes the Bearer-challenge test return `EdgeGate`; requiring configured Basic makes the missing-credential test return `UpstreamRejected`.
