...
- `wbs-client.test.ts`: treating any `WWW-Authenticate` challenge as the deployment
  gate misclassifies a Bearer challenge; only configured Basic is `EdgeGate`.
- `oauth.test.ts`: returning provider failures locally loses the validated redirect,
  client state, and one-shot `prompt=login`; accepting a modified marker makes the
  tamper assertion fail; skipping refresh-token revocation leaves the recorded revoke
  list empty.
