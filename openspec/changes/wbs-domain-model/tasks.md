...
- [x] 8.1 Built with TanStack Table. **It was first built without it, and that was wrong.** The argument — "be-01 returns rows in tree order, so there is no table state to own" — was only true because expand/collapse had been left out, and a WBS gets collapsed constantly. TanStack owns exactly one thing here: which branches are open. Ordering is still the server's, because the numbering is built so one lexicographic sort produces tree order at every level, and re-sorting on the client would be a second implementation of that.
- [x] 8.1a Expand and collapse, with `toTree` attaching children to parents from the flat list. A work item whose parent is missing is kept at the root rather than dropped — losing a row silently is worse than showing it in the wrong place, and there is a test for it.
- [x] 8.1b **The column definitions must not depend on frequently-changing state.** They first included `busy`, and `flexRender` renders each `cell` function as a component _type_ — so every busy flip gave every cell a new type and React unmounted and remounted the lot, silently eating the focus that Enter-Enter-Enter depends on. Found by printing what the focus ref actually saw after two guesses at the cause were wrong. Row buttons no longer disable; the toolbar outside the table still does.
- [x] 8.1c Focus moved from an effect to a callback ref, which fires exactly when the new row's input mounts rather than racing a later render.
