...
- [x] 1.1 `frameLayout(leafIds, state)` (T2's, already the single width
...
- [x] 1.1a The folded minimum is asked about real columns: T2 left
...
- [x] 1.2 `floorFor(columnId)` = `min(resolved default, 36)` and
      ...
      — **as built**: the clamp cases are in `table-frame.test.ts` and the
      "accepts what the clamp produces" case is in `wbs-table.test.tsx`, named
      "applies a width dragged as far right as it goes, on the reload after
      it", because the check being asserted lives there and seeding it from
      `widthFromDrag(...)` is what makes it one constant rather than two
- [x] 1.3 `floorFor` and the override lookup **throw** for a flexible column,
...
- [x] 1.4 Width state lives beside `expanded`, **not** in the `columns` memo:
...
- [x] 2.1 `ColumnResizeHandle` rendered on the trailing edge of every header
      ...
      failing
- [x] 2.2 The drag writes through `floorFor`/`WIDEST_COLUMN` and commits the
...
- [x] 3.1 `widthOverridesKey(projectId)` → `wbs.columnWidths.<projectId>`, and
...
- [x] 3.2 Per-entry validation, each rule a separate line with its own
      ...
      — **as built, (b) is not a line of its own.** It was written as
      `Number.isFinite(width)` and its negative was watched with the line
      deleted: it **passed**. `1e999` is the only non-finite width JSON can
      express, it parses to `Infinity`, and `Infinity` is above every ceiling
      exactly as `-Infinity` is below every floor — JSON has no `NaN` for the
      case the line would have been about. Per §0 the line was removed rather
      than believed. The range check is what refuses both, and both cases watch
      it: the non-finite one failed on `expected '' to be '56px'` with the range
      check deleted. R5; `P phases-ui`'s sanitizer, same shape
- [x] 3.3 The key is written on drag commit and read on mount; the sanitized
...
- [x] 4.1 An override outranks the resolved default, including a dynamic one:
...
- [x] 4.2 Reset **removes the key** and drops every override — the column
      ...
      **new** default; negative: reset re-written to store the current
      ...
      on the old width
- [x] 4.3 The reset is offered only while an override is in force — test:
...
- [x] 5.1 The reset renders inside the table renderer's own header region and
...
- [x] 6.1 `e2e/layout.spec.ts`: at `NARROW` with the frame scrolled, drag the
      ...
      **both** — the column's laid-out rectangle moved by the delta, and the
      ...
      — **as built the fault is a different one, because the named fault is not
      browser-only**: `table-frame.test.ts`'s `lays out, adds up, folds and pins
from the one number it resolved` already watches the pinned arm summing
      defaults, in jsdom. The fault this browser case was watched under is the
      one only a browser can see: the handle's
      `pointerdown`/`pointermove`/`pointerup` handlers removed, leaving the
      strip rendered and inert. It failed on `expected 169px to be 209px` while
      all 955 jsdom tests stayed green
- [x] 6.2 Reload with the same account and project: the dragged width is still
      there — fault: the drag-commit write removed → watched failing
- [x] 6.3 Press the reset: the column and the pinned offsets return to the
      resolved defaults — fault: reset storing a snapshot → watched failing
- [x] 6.4 `layout.spec.ts`'s existing imports and assertions are re-derived
      through `frameLayout` where T2 left them on constants, and the fit matrix
      re-runs at the widths it already covers

## 7. Gate

- [x] 7.1 `bunx nx format:check --all` and
...
- [ ] 7.2 Deploy to dev and Dany looks — the widths are a judgement call about
      ...
      **Not done, and not doable from here**: the work is on a branch that has
      not been pushed or merged, and `bin/dev-deploy.sh` runs from h1claw
      against a pushed checkout
