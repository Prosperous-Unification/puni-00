...
| PR #64 open                                         | 2026-08-14 18:56 |

**Branch cut to PR open: 24 minutes.** The read this time was smaller than
...
**Run 31831014308** on head `4a34b58` — **failure**. `pixels` succeeded;
`gate` failed on `Format`, one path: this file. The same trap `chart-clamp-words`
(#63) named in its own verify.md: `nx format:check` was run locally before
this file's last edit, and CI's `format:check --all` catches what the local
...
_only_ the wrong number — but the number itself is wrong for that one state.
