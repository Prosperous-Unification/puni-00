...
after that draws every bar a day short. **A Mermaid parse of the block SHALL
report believing the declaration** — the parsed diagram's own inclusivity flag
SHALL read true — not merely that the declaration's keyword is present in the
emitted text.

**A slice whose span crosses an excluded day (a weekend) SHALL parse to the
exact dates the block declared for it**, neither extended nor shifted by the
exclusion. Every task's end is a literal calendar date rather than a computed
duration, which is what keeps a parser's own weekend-extension behaviour from
ever applying to a task this block emits.
...
#### Scenario: a real Mermaid parse believes the declared inclusivity

- **GIVEN** any exported Mermaid gantt block
- **WHEN** a real Mermaid parser reads it
- **THEN** the parsed diagram SHALL report that end dates are inclusive

#### Scenario: a real Mermaid parse does not extend a bar across a weekend it already spans

- **GIVEN** a slice whose declared span crosses a weekend
- **WHEN** the exported block is read by a real Mermaid parser
- **THEN** the parsed task's start and end SHALL equal the dates the block
  declared, unmoved by the weekend exclusion
...
ids SHALL be generated. **A real Mermaid parse of an escaped name SHALL
recover the generated id intact** — the escaping SHALL be effective against
the grammar itself, not merely absent from the emitted string.
...
#### Scenario: a real Mermaid parse recovers the generated id past an escaped colon

- **GIVEN** a work item called `Phase 1: strip`
- **WHEN** the exported block is read by a real Mermaid parser
- **THEN** the parsed task's id SHALL be the writer's generated id, unmoved by
