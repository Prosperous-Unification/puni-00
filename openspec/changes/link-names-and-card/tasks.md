...
- [ ] 1.1 `PlanTableCellView` raises **every pinned column that opens a
...
- [ ] 1.2 The browser is the oracle for what the lift is _for_: a card that is
      not painted over.
...
- [ ] 2.1 `refLabelOf(url)` in `libs/domain/src/external-system/` beside
...
- [ ] 3.1 Migration `…_add_external_ref_name`: `ALTER TABLE
...
- [ ] 3.2 `schema.ts` gains the column with the JSDoc that says why a _typed_
      ...
      name read back.
- [ ] 3.3 `asOptionalExternalRefs` takes an optional `name`: a string, capped at
      `MOST_CHARACTERS_IN_A_REF_NAME`, absent meaning the empty string. A
      non-string or an over-long name is a typed 4xx
      (`externalRefs_entry_name_is_not_a_string`,
      `externalRefs_entry_name_is_too_long`), never a 500.
      Test: `work-item.routes.test.ts` — one case per refusal.
      Negative: each guard removed; watched failing on the status.
- [ ] 3.4 `revertTo` carries the name, so an undone replacement restores it.
      Test: `undo.test.ts` — `puts a ref's name back`.
      Negative: the field dropped from the inverse; watched failing on the name.
- [ ] 3.5 The wire shapes gain it: `work-item-response.ts` requires `name`,
...
- [ ] 4.1 `ExternalRefView` and `ExternalRefDraft` carry `name`;
      `setExternalRefsOf` already sends the draft whole.
- [ ] 4.2 The editor grows a Name box per stored row and on the add row. The add
...
- [ ] 5.1 `ExternalRefsCard` draws one row per ref: the family mark, the name
...
- [ ] 5.2 The browser is the oracle for the tint and the travel.
