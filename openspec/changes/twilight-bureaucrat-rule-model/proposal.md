...
Twilight Bureaucrat gains `check` and `explain`. A rule's mode comes from the consumer's trusted policy rather than code, and four checks the package already performs become registered rules with stable identifiers. A rule that could not be evaluated disallows the verdict in every mode. The sixteen existing routes remain unchanged, and a verdict never certifies.
...
- `bureaucrat-rules`: Registers existing checks as rules and reports policy-governed explanations and verdicts.
...
Only the `twilight-bureaucrat` project changes.
