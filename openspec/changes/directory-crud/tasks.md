...
- [ ] 1.1 `DirectoryService.renameTeam` + `PATCH /api/teams/:id` — trims,
...
- [ ] 1.2 `DirectoryService.patchPerson` + `PATCH /api/people/:id` — name
...
- [ ] 2.3 `DELETE /api/teams/:id` — same split; cascade nulls
      `work_item.service_team_id` on every carrier across projects **before
      the team row is deleted** — the column holds a real FK to
      `service_team(id)` (verified 2026-08-09 via
      `PRAGMA foreign_key_list(work_item)` on replayed migrations; the
      Drizzle model's omission is corrected in the same change) and
      `foreign_keys=ON` rejects the delete otherwise (negative: with the
      nulling moved after the delete, the FK violation is observed by the
      test, watched, then restored), moves those work items'
