...
- Impact: **breaking for wait-0 callers**, deliberately. `HEAVY_LOCK_WAIT_SECONDS=0` is the
  ...
  while a live ticket is ahead of it, even with the lock free — that instant is the lottery. Such
  a caller queues by setting a budget.

**Queue observability** — new, non-breaking: `bin/with-heavy-lock.sh status` prints the holder's
pid and lane label, then each waiting ticket oldest first with age and remaining budget; a claim
prints `heavy lock: waited <n>s behind <k> tickets`; a refusal names who is ahead; the gate labels
its lane `gate:<sha>`.

**Dead waiters** — new: a ticket whose pid is gone, or whose budget expired, is removed by
whoever sees it, named on stderr.

## Non-Goals

One lane: no priority classes, no fairness beyond arrival order. No lock path override — its
absence stops a caller opting out of the lock. No cross-host locking, no `flock`.

## Constraints

Bash 3.2, no new dependency, exit codes unchanged: 75 contention, 70 unknown state. Every new
check ships a watched negative.

- assumed: no design interview was held; the controller ruled on these points.
- assumed: the queue is `<lock path>.queue`, reached through the existing path argument, never
...
`bin/heavy-lock-lib.sh`, `bin/with-heavy-lock.sh`, `bin/h2puni-gate.sh`, their tests, and every
agent lane on h2puni. No application, library or deploy path.
