...
The system SHALL build `twilight-bureaucrat@0.1.0` into an allowlisted tarball whose executable and record validation work without repository source, workspace aliases, Nx, or network access.
...
- **THEN** `twilight-bureaucrat --version` reports `0.1.0` and record validation matches the source fixture result
