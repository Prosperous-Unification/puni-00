...
A dependency SHALL mean that the predecessor's **anchor slice** — its first
slice in role order — finishes before the successor's **first** slice starts.
The predecessor's later slices SHALL be free to run in parallel with the
successor. Parent ends SHALL expand to their leaves exactly as they do today:
every leaf beneath the predecessor has its anchor finish before any leaf
beneath the successor starts.
...
be free to start before the anchor it waits for.

A zero-length anchor SHALL clear the moment it is reached: a predecessor whose
first role carries no estimate imposes only its own predecessors' wait.
...
#### Scenario: a zero-length anchor clears immediately

- **GIVEN** `A` waiting on nothing, with no `Dev` estimate and 4 days of `QA`,
  and `B` depending on `A`
- **THEN** `B` starts on day 0 while `A`'s `QA` runs 0→4
...
carries, never recomputed from estimates. A person link SHALL be drawn
...
- **WHEN** a predecessor's first role is unestimated and its anchor stands at
  day 6
