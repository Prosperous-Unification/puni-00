...
| foreign keys referencing it | `schema.ts:928,1277,1403,1553`                                                     | `work_item_team.team_id`, `project_team_capacity.service_team_id`, `team_service.team_id`, and `person_team.service_team_id` reference `service_team(id)` with `ON DELETE CASCADE` |
| migration-only foreign key  | `drizzle/20260806190000_add_teams_and_assignees/migration.sql:31`                  | `work_item.service_team_id` references `service_team(id)` with NO ACTION; `schema.ts:376` declares the column without `.references()`, so R2-6 must preserve this FK explicitly    |
