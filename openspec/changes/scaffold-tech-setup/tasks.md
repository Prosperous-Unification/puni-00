...
- [x] 5.3 Generate `libs/scripts` (`runtime:bun`). Export `$` wrapper over `Bun.$` with structured errors, SSH command builder, typed JSON/YAML readers, Dagger argument helpers. _(7 tests pass; added yaml@^2)_
...
- [x] 10.1 Generate `tools/tool-compose` with per-tier Caddyfile + Compose fragment templates (`be.caddy.tmpl`, `gw.caddy.tmpl`, `fe.caddy.tmpl`, `observability.caddy.tmpl`, and matching `*.compose.tmpl`). Targets: `build`, `render`. _(renderTemplate + renderAll with `{{PLACEHOLDER}}` substitution; 3 tests pass; per-tier templates seeded)_
- [x] 10.2 Generate `tools/tool-observability-stack` with `grafana/provisioning/{datasources,dashboards}/`, seed dashboards (`be-01-overview`, `gw-01-overview`, `wbs-alerts`), `prometheus.yml`, `promtail.yml` (labels `{service,level,version}`, structured-metadata `{request_id,connection_id,trace_id,span_id,ws_subscription}`), `loki.yml`. Targets: `build`, `validate` (JSON-schema + `promtool check`). _(validateTree parses JSON dashboards + YAML configs; scaffolded tree passes; promtool invocation left as a docs note — runs only when a real promtool binary is on `PATH`)_
- [x] 10.3 Generate `tools/tool-secrets` with placeholder `src/production.env.sops`, `src/local.env.example`, `src/README.md` (rotation playbook), and CLI in `src/cli/{decrypt,push,encrypt,updatekeys}.ts`. Root `.sops.yaml` with the first developer's age public key. Targets: `decrypt`, `push`, `encrypt`, `updatekeys` (all `cache: false`). _(CLI entrypoints are intentionally placeholders that print intended sops commands and refuse to run until `assertRealCiphertext` passes — respects the "no destructive operations on live infra" constraint)_
- [x] 10.4 Generate `tools/tool-git-hooks` with `src/install.ts` that runs `bunx lefthook install`, and pre-commit helpers in `src/hooks/` (plaintext-secret guard, optional migration lint). Target: `install`. _(hooks/{plaintext-secrets,migration-lint,conventional}.ts with 6 unit tests; `lefthook.yml` configures pre-commit + commit-msg stages; `install.ts` shells out to `bunx lefthook install`)_
