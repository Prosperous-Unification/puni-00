...
- **Cache identity:** one SQLite table `optimized_schedule_cache` in the existing application database, declared in `apps/be-01/src/repository/schema.ts` with a forward migration under `apps/be-01/drizzle/`. Composite primary key `(projectId, inputHash, objective, solverVersion)` → `status` (`'ok' | 'failed'`), `scheduleJson` (NULL when failed), `failureReason` (NULL when ok), `createdAt`. A row satisfies a read iff `status='ok'`, the hash matches the current input, and the version matches the installed package. Retention is one rule: a project keeps only its current generation's rows (`ok` and `failed` for the current hash) and a new generation deletes every older row for that project — so an undo back to a previous hash is a miss that re-solves, with Fast visible throughout. A `failed` row suppresses an automatic re-spawn for its exact key (a read must not become the timer retry the failure policy forbids) and never blocks an explicit Retry or a new hash's generation. **`apps/be-01/drizzle/**` is a prod-mode path, so the slice that lands this migration ships as a reviewed PR and is not self-merged.**
...
- **Resource ceilings:** 4 solver processes per project, 16 global (Dany 2026-09-02). When the global cap is full, entries wait in a single global FIFO ordered by enqueue time, one per (project, objective); at dequeue an entry is discarded without launching if its hash no longer matches its project's current hash **or** the project's toggle is no longer ON — the toggle is excluded from the hash, so the hash check alone would not catch an opt-out.
