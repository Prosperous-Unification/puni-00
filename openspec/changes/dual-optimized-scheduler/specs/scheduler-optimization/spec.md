...
#### Scenario: deleting a project releases its slot budget

- **GIVEN** a project holding an admitted solver slot and a queued entry
- **WHEN** the project is deleted
- **THEN** its generation, slot, queue and cache rows are gone and the global slot count reflects the release
...
A project with no slices, or one whose slices all carry a zero duration, SHALL NOT be sent to a solver. The maxima the optimized definition relies on — `MAKESPAN` as the maximum finish and `horizonUnits` as `max(notBeforeUnits) + Σ durationUnits` — have no empty-set identity, while `schedule` handles the empty plan explicitly and returns an empty schedule with a project finish of zero. The coordinator SHALL therefore short-circuit: no slot is allocated, no process is spawned, no cache row is written and no event is emitted, and the plan read SHALL return Fast with every variant reported `idle`.
