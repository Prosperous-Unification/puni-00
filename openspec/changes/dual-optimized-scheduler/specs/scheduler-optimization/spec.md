...
#### Scenario: a failed row never satisfies a read

- **GIVEN** a `status='failed'` row for the current input hash and objective
- **WHEN** that objective's schedule is read
- **THEN** the row is not returned as a schedule, Fast stays visible, and a Retry starts a fresh solve for the same key
