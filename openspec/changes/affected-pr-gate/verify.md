...
Every pass/fail count below — and in section 5 — is **as observed when that row was watched**,
so the totals differ between rows: cases were added to both suites across three rounds, and two rows were
...
**Commit range**: `73730b66..` the tip of `change/affected-pr-gate`. EVERY commit below
changes code except the three marked docs — the review rounds moved `ci.yml`, both
`project.json` files and three test files, so none of them is "documentation of the change".
...
| `243ad1b1`             | docs: the index describes the gate that now runs                      | `LLM_README.md`                                                                      |
...
| `2782ee67`             | test(ci): make the refusal-arm pins breakable again                   | `ci.yml`, both pin suites, `workspace-targets.test.ts`                               |
