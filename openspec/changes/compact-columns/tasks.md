...
- [ ] 1.1 `frameLayout(leafIds: readonly string[], state: FrameLayoutState)` in
...
- [ ] 1.2 The two guards move onto the derived path and stay breakable — an
...
- [ ] 1.3 `foldedTableMinWidth(roleIds, state)` — the project's **real** role
...
- [ ] 1.4 `wbs-table.tsx`'s `<colgroup>`, the table's `min-width` and
...
- [ ] 2.1 `short-date.ts`: `shortIsoDate(iso, today)` reading year, month and
...
- [ ] 2.2 `shortInstant(epochMs, now)` in the browser's own zone, same two
...
- [ ] 2.3 Start and End cells print `shortIsoDate` with the full ISO in
...
- [ ] 3.1 `DateField` grows `onExit('commit' | 'cancel')`: Enter commits then
...
- [ ] 3.2 Escape reports the exit and commits nothing of its own — test:
...
- [ ] 3.3 The `not-before` cell renders the short date at rest (em-dash for
...
- [ ] 3.4 Closing an editor returns the focus to its cell, and the cell keeps
      its `data-cell` and the table's Tab handling — test:
      `wbs-table.test.tsx` focus assertions per exit route
- [ ] 3.5 A refetch mid-edit follows the grid's refused-draft rules: the
...
- [ ] 3.6 Chromium proves what jsdom cannot perform: Escape after typing leaves
...
- [ ] 4.1 `frameLayout` resolves `not-before` to 84 while any row **in the
...
- [ ] 4.2 The Number width oracle measures the **envelope**, not an unbounded
...
- [ ] 4.3 Past the envelope the cell clips and tells the truth: the Number cell
...
- [ ] 5.1 `measure()` no longer requires a first-row `input[type="date"]`: it
...
- [ ] 5.2 The intrinsic-width assertion keeps its teeth on the edited state —
...
- [ ] 5.3 The fit matrix re-runs across the existing viewports with the new
...
- [ ] 6.1 `plan-cards.tsx` renders spans through the same formatter the columns
