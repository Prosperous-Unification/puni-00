...
| `58f5d87` | nothing — the head the code ships on                             | 31454062093 | `gate` success, `pixels` success                            |
...
On `58f5d87` both jobs concluded successful: `pixels` green on all five dependency-hover checks — the two-palette direction check and the keyboard check included, the two that had no browser proof before this round — and `gate` green on the run-many with `build` in it. `gate` failed once on that head before passing, and not on this change: `gw-01`'s presence-roster fan-out test timed out on `no roster arrived for ada, grace` (`fan-out.integration.test.ts:165`), the same websocket flake the local run-many hit and the same one `deps-single-line` recorded. The failed job alone was re-run and passed; `pixels` was green first time and was not re-run. The flake is gw-01's to chase and is worth chasing — three occurrences in one afternoon across two machines — but nothing in this change reaches it.

**This commit is the record and nothing else.** It changes `verify.md` alone, so the green above stands for the code exactly as it ships; the precedent is `gantt-calendar-snap`'s own "record CI green on the review-fixes head".
