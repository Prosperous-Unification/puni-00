...
- [x] 6.1 `bin/h2puni-gate.sh`, `openspec validate --all --json`, and the **whole** `CI=1` Playwright gate on shifted ports were all run on 2026-08-30 and are quoted in `verify.md` with their real output. `openspec` is 91/91. The browser gate is 229 passed / 4 failed / 1 skipped; none of the four is reachable from this branch's diff, and one of them — `phases.spec.ts:204` — is a live regression `estimate-triple-visible` left on `main`, reported rather than repaired. The repo gate's `format:check`, and every `test`, `lint`, `typecheck` and `build` target for the three apps and the libs, pass; three `tools/*` test targets fail on macOS host tooling and are untouched by this change's diff.
