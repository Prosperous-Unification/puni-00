...
- [ ] 1.1 `GANTT_MIN_PX = 84` (the axis row and two chart rows, `3 * ROW_PX`)
...
- [ ] 1.2 `GanttPanel` takes `heightPx: number | null`; `null` keeps
...
- [ ] 2.1 `rememberedGanttHeight(projectId): number | null`,
...
- [ ] 2.2 A write happens on commit and at no other time — opening a project
...
- [ ] 3.1 A handle rendered by the shell **above and outside**
...
- [ ] 3.2 The handle stands when the chart cannot be drawn: it is outside the
...
- [ ] 4.1 `resetColumnWidths` becomes `resetLayout`: forgets both keys, drops
...
- [ ] 4.2 The reset moves into the toolbar row as `data-toolbar`'s own child —
...
- [ ] 5.1 `e2e/gantt.spec.ts`: a real drag on the separator makes the panel
...
- [ ] 5.2 Gate: `bunx nx format:check --all` and
