...
| 7   | `flexibleCellStyle` flooring the cell at the override          | the override dropped from the floor                                                                | `table-frame.test.ts` — `lays out, adds up, folds and pins a dragged Name…`, **and on the rendered production path** `wbs-table.test.tsx` — `lays a remembered Name width on the table itself…` | `expected { minWidth: 200 } to deeply equal { minWidth: 300 }`; rendered: `expected '200px' to be '300px'`, the header Name cell's own min-width                 |
| 8   | `sizableColumn`'s flexible arm                                 | the arm deleted                                                                                    | `table-frame.test.ts` — `says which ids can be sized at all…`, **and on the storage production path** `wbs-table.test.tsx` — `lays a remembered Name width on the table itself…`                | `expected false to be true`; storage: `expected '200px' to be '300px'` — the stored `name` entry silently dropped, the table opening undragged                   |
...
## Known limitation, recorded by decision

**Version rollback drops a `name` width on an old client's next drag.** A
client running the pre-`name-column-drag` sanitizer that loads this change's
stored blob keeps every entry it can read but treats `name` as unsizable and
drops it from the in-memory map — and because a drag commit persists the
sanitized map wholesale, the reader's next resize of **any** column silently
erases the `name` entry from storage. Decision (coordinator, 2026-08-10):
this is preference loss on version rollback, not data loss — localStorage
widths are per-client and disposable, and a schema-version preservation
mechanism for a width preference is out of proportion. Recorded; not built.
