...
| 10  | the container's own top dropped from `roomForCard`'s `Math.max`         | `gives a card no room rather than less than none when its frame is off screen` | `expected { side: 'below', maxHeight: -180 } to deeply equal { side: 'below', maxHeight: 0 }`                   |
