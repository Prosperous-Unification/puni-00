...
- [x] 1.1 `libs/domain/src/effective-team.ts`: `effectiveTeamsOf` answering
...
- [x] 1.2 `effective-team.test.ts` rewritten rather than adapted: the whole set
...
- [x] 1.3 The cycle guard and the memoisation, both re-derived: the cycle test
...
- [x] 2.4 The cascade, and the outgoing release's own writes: a team removed by
...
- [x] 3.1 `WorkItemRepository.listByProject` returns `LabelledWorkItem` — the row
      plus `teamIds` ordered by team id (D6). Test: two join rows come back as
      two, in that order.
- [x] 3.2 `patch` writes the column and the join in one transaction, and clearing
...
- [x] 3.3 `insert` writes the join row for a row that arrives labelled — the
      restore/copy parity D2 argues for. Test: a labelled insert reads back with
      its team.
- [x] 3.4 `SubtreeRepository.insertSubtree` writes join rows for every copied
      row. **Negative:** the derivation deleted, watched failing the duplicated
      branch's team.
- [x] 3.5 `directory.ts`: `usageRowsIn`, `removeTeam`'s in-use count and
...
- [x] 4.1 `poolFor(teamIds, teamSizes)` exported from `work-item.service.ts`:
...
- [x] 4.2 `tree()` reads `effectiveTeamsOf` and the payload carries `teamIds` per
      row. `NumberedWorkItem` gains the field with its JSDoc.
- [x] 4.3 `directoryUsageOfTeam` reads the set both for `label_nulled` (own set)
...
- [x] 4.4 Claim B: in `service/capacity-migration-identity.test.ts` rather than a
      second file — one oracle, captured before both changes, measures both, and
      a copied 150-line replay harness would be the thing that drifts. The
      committed oracle replayed with `teamIds` lifted off and asserted separately as the singleton of
...
- [x] 5.1 `WorkItemView.teamIds`, `TreeRow` through it, and the table's
...
- [x] 5.2 `plan-export.ts`: `ExportRow.teamIds`, `teamsInForce` over the new
      ...
      here as well).
- [x] 5.3 Every fe-01 fixture that builds a work item row by hand carries
...
- [x] 6.1 `CONTEXT.md`: **team set** and **effective team set** as terms.
- [x] 6.2 `verify.md`: the gate's numbers from h2puni, the CI run, and the
