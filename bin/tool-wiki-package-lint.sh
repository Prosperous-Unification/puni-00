...
# <revision>` interface to the root-installed `twilight-burokrat lint`. It replaces that path
# once the root pin lands; until then the root has no pin and this refuses (runbook, "Flip").
set -euo pipefail

if [[ -n ${TWILIGHT_BUROKRAT_COMPAT_ROUTE:-} ]]; then
  ...
  printf 'tool-wiki lint: compatibility route re-entered itself through %s\n' \
    "$TWILIGHT_BUROKRAT_COMPAT_ROUTE" >&2
  exit 70
fi
...
if ! pin=$(
  env -i PATH="$(dirname -- "$bun_path"):/usr/bin:/bin" MANIFEST="$repo_root/package.json" \
    ...
    const pin = manifest.devDependencies?.["twilight-burokrat"];
    if (typeof pin !== "string" || !/^[0-9]+\.[0-9]+\.[0-9]+$/.test(pin)) process.exit(1);
    process.stdout.write(pin);
  '
); then
  printf 'tool-wiki lint: root package.json has no exact twilight-burokrat devDependency\n' >&2
  exit 78
fi
installed="$repo_root/node_modules/twilight-burokrat"
if ! installed_root=$(realpath -- "$installed") ||
  [[ $installed_root != "$repo_root/node_modules/"* ]]; then
  printf 'tool-wiki lint: twilight-burokrat is not installed; run bun install\n' >&2
  exit 78
fi
executable="$installed_root/dist/bin.mjs"
if ! version=$("$bun_path" --no-env-file "$executable" --version) || [[ $version != "$pin" ]]; then
  printf 'tool-wiki lint: installed twilight-burokrat is not the pinned %s\n' "$pin" >&2
  exit 78
fi
TWILIGHT_BUROKRAT_COMPAT_ROUTE=${BASH_SOURCE[0]} exec "$bun_path" --no-env-file "$executable" lint "$@"
